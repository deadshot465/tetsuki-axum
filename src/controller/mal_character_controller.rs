...
#[get("/mal_character")]
async fn get_all_mal_characters() -> impl Responder {
    let (_client, database) = initialize_clients();
    let collection = database.collection_client(MAL_CHARACTERS);
    let query_result = inner_get_all_mal_characters(&collection).await;
    HttpResponse::Ok().json(query_result)
}

#[get("/mal_character/{id}")]
async fn get_mal_character(id: Path<i32>) -> impl Responder {
    ...
    let query_result = query_document::<MalCharacter, _, _>(MAL_CHARACTERS, query, true)
        .await
...
#[post("/mal_character")]
async fn post_mal_character(payload: actix_web::web::Json<MalCharacter>) -> impl Responder {
    let mut payload = payload.into_inner();
    if payload.id.is_empty() {
        payload.id = Uuid::new_v4().to_string()
    }

    match add_document(MAL_CHARACTERS, payload.clone()).await {
